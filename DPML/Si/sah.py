...
class Sah():
    ...
    def __init__(self,c,d1,d2,dnrange):
        ...
        # self.tauSah = [float((1+(Decimal(c.p0)+Decimal(dn)+Decimal(d1.k*self.n1_1))/(Decimal(self.p1_1)+Decimal(d1.k)*(Decimal(c.n0)+Decimal(dn)))+(Decimal(self.p1_2)+Decimal(d2.k)*(Decimal(c.n0)+Decimal(dn)))/(Decimal(c.p0)+Decimal(dn)+Decimal(d2.k*self.n1_2)))/((Decimal(c.n0)+Decimal(c.p0)+Decimal(dn))*(1/(Decimal(self.taun0_1*self.p1_1)+Decimal(self.taup0_1)*(Decimal(c.n0)+Decimal(dn)))+1/(Decimal(self.taun0_2)*(Decimal(c.p0)+Decimal(dn))+Decimal(self.taup0_2)*Decimal(self.n1_2))))) for dn in dnrange] #missing thermal velocity - Sijin Wang
        self.tauSah = [float((1+((Decimal(c.p0*c.Vp)+Decimal(dn*c.Vp))+Decimal(d1.k*self.n1_1*c.Vn))/(Decimal(self.p1_1*c.Vp)+Decimal(d1.k*c.Vn)*(Decimal(c.n0)+Decimal(dn)))+(Decimal(self.p1_2*c.Vp)+Decimal(d2.k*c.Vn)*(Decimal(c.n0)+Decimal(dn)))/((Decimal(c.p0)+Decimal(dn))*Decimal(c.Vp)+Decimal(d2.k*self.n1_2*c.Vn)))/((Decimal(c.n0)+Decimal(c.p0)+Decimal(dn))*(1/(Decimal(self.taun0_1*self.p1_1)+Decimal(self.taup0_1)*(Decimal(c.n0)+Decimal(dn)))+1/(Decimal(self.taun0_2)*(Decimal(c.p0)+Decimal(dn))+Decimal(self.taup0_2)*Decimal(self.n1_2))))) for dn in dnrange]
